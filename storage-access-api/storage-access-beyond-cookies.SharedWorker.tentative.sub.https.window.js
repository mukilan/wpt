...
// Step 8 (top-frame) Set up count worker to expect it's already opened.
...
async_test(t => {
  ...
  relay_worker.port.onmessage = t.step_func(e => {
    ...
    // Step 8
    const count_worker = new SharedWorker("/storage-access-api/resources/shared-worker-count.js", {name: id, sameSiteCookies: 'none'});
    count_worker.port.onmessage = t.step_func(e => {
      assert_equals(e.data, 2, "Count worker should have already been opened");
      t.done();
    });
  });
